...
func (p *parser) must(t lex.Type) (*lex.Lexeme, *typ.Error) {
	...
	if p.err == nil && p.cur.Typ != t {
		...
		p.err = &typ.Error{
			...
			Snip: []*tty.Snippet{{File: p.src, Span: p.cur.Tok}},
		}
	}
...
func Parse(pkg *mod.Package, opts ...Option) bool {
	ops := &options{}
	...
	return parse(pkg, ops)
}

func parse(pkg *mod.Package, ops *options) bool {
	...
	for _, src := range pkg.Src {
		...
		dec.Box.Position().Row = 1
		dec.Box.Position().Col = 1
		dec.Box.Render()
...
func (p *parser) parseDecl() (Decl, *typ.Error) {
	...
	case lex.USE:
		...
		if pkg == nil {
			...
			return nil, &typ.Error{
				...
				Snip: []*tty.Snippet{{File: p.src, Span: use.Span()}},
			}
		}
	...
	default:
		...
		tok.Tok.Hint().Text = fmt.Sprintf("%d:%d - declaration expected here",
			tok.Tok.Pos.Row,
			tok.Tok.Pos.Col)

		return nil, &typ.Error{
			Full: "unexpected control sequence",
			Snip: []*tty.Snippet{{File: p.src, Span: tok.Tok}},
		}
	}
}
...
func (p *parser) parseFuncDecl() (*FuncDecl, *typ.Error) {
	...
	if tok.Typ == lex.LB {
		...
		if err != nil {
			dec.Box.Add(err.Snip[0].Span, Indent, 0)
			err.Snip[0].Span = dec.Box
			return nil, err
		}
...
func (p *parser) parseTypeSpec(g tty.Group, r, b int) (*TypeSpec, *typ.Error) {
	...
	tok.Tok.Hint().Attr.Color.Add(color.FgRed)
	tok.Tok.Hint().Text = fmt.Sprintf("%d:%d - type specification expected here",
		tok.Tok.Pos.Row,
		tok.Tok.Pos.Col,
	)
...
func (p *parser) parseBlockStmt() (*BlockStmt, *typ.Error) {
	...
	if tok.Typ != lex.RB {
		...
		for {
			...
			if err != nil {
				res.Box.Add(err.Snip[0].Span, 0, 0)
				err.Snip[0].Span = res.Box
				return nil, err
			}
...
func (p *parser) parseStmt() (Stmt, *typ.Error) {
	...
	case lex.IDEN:
		...
		if tok.Typ == lex.LP {
			...
			if err != nil {
				res.Box.Append(err.Snip[0].Span.(tty.Mono), 0)
				err.Snip[0].Span = res.Box
				return nil, err
			}

			res.Box.Append(tok.Tok, 0)
		...
		if err != nil {
			switch err.Snip[0].Span.(type) {
			case *tty.Box:
				res.Box = &tty.Box{}
				res.Box.Add(top, 0, 0)
				res.Box.Add(err.Snip[0].Span, Indent, 0)
			case tty.Mono:
				res.Box = &tty.Row{}
				res.Box.Add(top, 0, 0)
				res.Box.Add(err.Snip[0].Span, 1, 0)
			}
...
func (p *parser) parseExpr0() (Expr, *typ.Error) {
	...
	for {
		...
		if err != nil {
			...
			box.Append(err.Snip[0].Span.(tty.Mono), 1)
		...
		case lex.LAND, lex.LOR:
			...
			if err != nil {
				switch yb := err.Snip[0].Span.(type) {
				case *tty.Box:
					yb.Prepend(tok.Tok, 1)
...
func (p *parser) parseExpr7() (Expr, *typ.Error) {
			...
			if tok.Typ != lex.RB {
				...
				for tok.Typ != lex.RB {
					...
					if tok.Typ != lex.RB {
						...
						if err != nil {
							mem.Box.Append(err.Snip[0].Span.(tty.Mono), 1)
							err.Snip[0].Span = res.Box
							return nil, err
						}

						mem.Box.Append(com.Tok, 0)
					}
				}
			} else {
				res.Box = top
	...
	tok.Tok.Hint().Attr.Color.Add(color.FgRed)
	tok.Tok.Hint().Text = fmt.Sprintf("%d:%d - expression expected here",
		tok.Tok.Pos.Row,
		tok.Tok.Pos.Col,
	)
