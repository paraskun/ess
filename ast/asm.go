...
func Assemble(p *mod.Package) *co2.Package {
	a := assembler{pkg: &co2.Package{
		Text: make([]byte, 0),
		Data: make([]byte, p.Dsz),

		Sym: make([]*co2.Symbol, 0),
		Rel: make([]*co2.Relocation, 0),
	}}
	...
	for n, o := range e.Sym {
		...
		default: // variable
			...
			copy(a.pkg.Data[o.Off:], bin)

			a.pkg.Sym = append(a.pkg.Sym, &co2.Symbol{
				Sym: n,
				Off: o.Off,
				Seg: typ.Package,
			})
